...
# parent_dir = str(Path("Gridworld MDP").resolve().parent.parent)
...
class GRDModel:
    def __init__(self, grid_size = 6, start_pos = (0,0), goal_positions = [],blocked_positions = [], unblocked_positions = [],
        ...
        if compute_wcd:
            ...
            self.init_encoding= encode_grid_design(grid_size, goal_positions, blocked_positions, start_pos).float().to(COMPUTE_DEVICE) # will be useful in computing predicted
        self.n_changes_so_far = n_changes_so_far #[n_blockings,n_unblockings]
            
    
        
    def reduce_exhaustive(self,all_modifications_uniform):
        children = []
        ...
        if self.n_changes_so_far[1] < self.n_max_changes[1] or all_modifications_uniform: # still more room to unblock   
            for action in self.blocked_positions:
                ...
                child = GRDModel(grid_size = self.grid_size, start_pos = self.start_pos,
                ...
                is_valid, cost_to_goals = is_design_valid(self.grid_size, self.goal_positions, 
                                                          blocked, self.start_pos) # checks iif the design is valid and computes the shortest path to each goal
    ...
    def greedy_next_true_wcd(self,all_modifications_uniform):
        ...
        if (self.n_changes_so_far[0] < self.n_max_changes[0]) or all_modifications_uniform: # still more room to block
            for action in self.unblocked_positions:
                ...
                child = GRDModel(grid_size = self.grid_size, start_pos = self.start_pos,goal_positions = self.goal_positions,
                                 blocked_positions = blocked, unblocked_positions = unblocked,
                                 init_goal_costs=self.init_goal_costs, 
    ...
    def compute_true_wcd(self):
        return compute_wcd_single_env_no_paths(self.grid_size, self.goal_positions, self.blocked_positions, self.start_pos, vis_paths = False, return_paths = False)
...
class GRDModelBlockingOnly:
    def __init__(self, grid, paths,blocked = [],compute_wcd = True, fixed_positions = None, n_changes_so_far = 0,init_goal_costs=[]):
        # assert wcd_paths[0] in paths[0] and wcd_paths[1] in paths[1]
        self.grid = grid
        self.paths = paths
        self.blocked = blocked
        self.init_goal_costs = np.array(init_goal_costs)
        ...
        if compute_wcd: # compute True WCD
            ...
            self.init_encoding= encode_grid_design(self.grid_size, self.goal_positions, self.blocked, self.start_pos).float().to(COMPUTE_DEVICE) 
            # will be useful in computing predicted
    ...
    def reduce_prune_reduce(self):
        children = []
        # print("Called", self.get_wcd(), len(self.get_unique_actions(self.wcd_paths)))
        #get unique actions in wcd_paths and create GRD models for each of them
        for action in self.get_unique_actions(self.wcd_paths):
            # print("Action:", action)
            if not ("G" in self.grid[action[0],action[1]] or "S" in self.grid[action[0],action[1]] ): # start or goal state cant be blocked
                ...
                if not (local_paths is None): # there are paths to each goal
                    child = GRDModelBlockingOnly(self.grid,local_paths, self.blocked+[action] ,n_changes_so_far = self.n_changes_so_far+1,
                                                 fixed_positions = self.fixed_positions,init_goal_costs=self.init_goal_costs)

                    cost_to_goals = child.get_shortest_paths()

                    if (np.array(cost_to_goals)<= self.init_goal_costs).all() and (child.get_wcd() <= self.get_wcd()):
                        children.append(child)
    ...
    def get_shortest_paths(self):
        dists = []
        for paths_to_goal in self.paths:
            dists.append(np.min([len(path) for path in paths_to_goal]))
    ...
    def greedy_next_true_wcd(self):
        ...
        children = []
        unique_actions = self.get_unique_actions(self.paths)
        counter = 0
        # get unique actions in wcd_paths and create GRD models for each of them
        ...
        for action in unique_actions:
            ...
            if not ("G" in self.grid[action[0],action[1]]): # goal state cant be blocked
                ...
                if not local_paths is None: # there are paths to each goal
                    ...
                    child= GRDModelBlockingOnly(self.grid,local_paths, self.blocked+[action],
                                                n_changes_so_far = self.n_changes_so_far+1,
                                                fixed_positions = self.fixed_positions, init_goal_costs=self.init_goal_costs )
                    
                    cost_to_goals = child.get_shortest_paths()
                        
                    if (child.get_wcd()<best_wcd) and ((np.array(cost_to_goals)<= self.init_goal_costs).all()):
                        next_child = child
                        best_wcd = child.get_wcd()
        return next_child
    ...
    def greedy_next_pred_wcd(self,model):
        ...
        for action in self.get_unique_actions(self.paths):
            if not ("G" in self.grid[action[0],action[1]]): # goal state cant be blocked
                ...
                if not local_paths is None: # there are paths to each goal
                    child= GRDModelBlockingOnly(self.grid,local_paths, self.blocked+[action],n_changes_so_far = self.n_changes_so_far+1,
                                                compute_wcd= False,init_goal_costs=self.init_goal_costs)
                    child_wcd = child.compute_predicted_wcd(model)
                    cost_to_goals = child.get_shortest_paths()
                    
                    if child_wcd<best_wcd and ((np.array(cost_to_goals)<= self.init_goal_costs).all()):
                        next_child = child
                        best_wcd = child_wcd
...
def breadth_first_search_blocking(root, max_budget,use_exhaustive_search = False):
    ...
    while queue:
        ...
        if current_node.get_wcd() == 0:
            # If the goal state with WCD = 0 is reached, return immediately
            return current_node
        ...
        if lowest_wcd_node is None or wcd < lowest_wcd_node.get_wcd():
            lowest_wcd_node = current_node
                
        if np.array(current_node.get_num_changes_so_far()).sum() < np.array(max_budget).sum():
            # If the current node is not a goal state and within budget, expand the search
    ...
    # If no goal state with WCD = 0 is found, return the node with the lowest WCD
...
def greedy_search_true_wcd_blocking_only(root, max_budget):
    ...
    while True:
        ...
        if current_node.__eq__(parent):
            ...
            return current_node
        print("used Budget",current_node.get_num_changes_so_far(), "Max :", max_budget)
    return parent
...
def breadth_first_search_all_actions(root, max_budget,all_mods_uniform = False):
    ...
    while queue:
        ...
        wcd, children = current_node.reduce_exhaustive(all_mods_uniform)
        if wcd < lowest_wcd:
            lowest_wcd = wcd
            lowest_wcd_node = current_node
            
        if all_mods_uniform and (np.sum(current_node.get_num_changes_so_far()) >= np.sum(max_budget)): # budget used up
            continue

        elif (not all_mods_uniform ) and (np.array(current_node.get_num_changes_so_far())>= np.array(max_budget)).all(): # budget used up
            continue
