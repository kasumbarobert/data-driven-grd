...
class OvercookedEnv(object):
    ...
    def mp(self):
        if self._mp is None:
            if self._mlam is not None:
                ...
                if self.info_level > 0:
                    print_message = "Computing MotionPlanner"
                    # print(print_message)
    ...
    ####################
    # ENVIRONMENT FEATURIZATION #
    ####################

    def featurize_environment(self):
        """
            - Returns a featurized representation of the environment 
            - The object distance is computed as the manhattan distance 
        """
        terrain_dict = self.mdp._get_terrain_type_pos_dict()
        terrain_objects = ["P","O","T","S","D"," ","X"]
        feature_dict ={}
        pot_pos = terrain_dict["P"][0]
        print(pot_pos)

        for obj in terrain_objects:

            if obj =="X" or obj ==" ": # Just store the count of Xs or " " - Ignoring the state 
                feature_dict["prop_"+ (obj if obj=="X" else "space") +"s"] = len(terrain_dict[obj])/(self.mdp.width*self.mdp.height)
            else:
                feature_dict[obj+"_dist_from_P"] = manhattan_distance(terrain_dict[obj][0], pot_pos) # score 1D index of the object position

        feature_dict["o_t_distance"] = manhattan_distance(terrain_dict["O"][0], terrain_dict["T"][0])
        count = 1
        recipe_config = Recipe.configuration
        for order in recipe_config["all_orders"]:
            o_t_counts = dict(Counter(order["ingredients"]))
            if "onion" in o_t_counts.keys():
                feature_dict[f"order_{count}_num_o"]= o_t_counts["onion"]
            else:
                feature_dict[f"order_{count}_num_o"] =0
            if "tomato" in o_t_counts.keys():
                feature_dict[f"order_{count}_num_t"]= o_t_counts["tomato"]
            else:
                feature_dict[f"order_{count}_num_t"] = 0

            feature_dict[f"order_{count}_reward"]= recipe_config["recipe_values"][count-1]
            feature_dict[f"order_{count}_time"]= recipe_config["recipe_times"][count-1]

            count+=1

        feature_dict["max_num_ingredients"] = recipe_config["max_num_ingredients"]
        feature_dict["grid_size"] = self.mdp.width*self.mdp.height
        return feature_dict
