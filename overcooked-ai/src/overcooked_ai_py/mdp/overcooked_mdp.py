...
class Recipe:
    ...
    @classmethod
    def update_config(cls,conf):
        if not cls._configured:
            raise ValueError("Recipe class not yet configured")
        old_conf = Recipe.configuration
        # Mutual Exclusion
        ...
        if 'recipe_values' in conf:
            ...
            if not len(old_conf['all_orders']) == len(conf['recipe_values']):
        ...
        if 'recipe_times' in conf:
            ...
            if not len(old_conf['all_orders']) == len(conf['recipe_times']):
        ...
        if 'recipe_values' in conf:
            cls._value_mapping = {
                cls.from_dict(recipe) : value for (recipe, value) in zip(old_conf['all_orders'], conf['recipe_values'])
            }
            

        if 'recipe_times' in conf:
            cls._time_mapping = {
                cls.from_dict(recipe) : time for (recipe, time) in zip(old_conf['all_orders'], conf['recipe_times'])
            }
...
class SoupState(ObjectState):
    ...
    def __hash__(self):
        ingredient_hash = hash(tuple([hash(i) for i in sorted(self._ingredients, key=lambda o: o.name)])) # the sorting ensures that 1T2O == 2O1T
        supercls_hash = super(SoupState, self).__hash__()
        # return hash((supercls_hash, self._cooking_tick, ingredient_hash))
        return hash((supercls_hash,self.is_cooking, self.is_ready, ingredient_hash)) # modified to ignore the specific self._cooking_tick,
    ...
    def add_ingredient(self, ingredient):
        ...
        self._ingredients = sorted(self._ingredients,key=lambda o: o.name) # to ensure that ordering of ingredients doesnt matter
...
class OvercookedState(object):
    ...
    def __init__(self, players, objects, bonus_orders=[], all_orders=[], timestep=0, soup_delivered = False,  **kwargs):
        """
        ...
        self.soup_delivered = soup_delivered
    ...
    def mark_soup_delivered(self):
        self.soup_delivered = True
    ...
    def time_independent_equal(self, other): # compares if the two OvercookedStates are equal without considering time
    ...
    def __eq__(self, other): # compares if the two OvercookedStates are equal considering time
    ...
    def __hash__(self):
        ...
        return hash(
            (self.players, tuple(self.objects.values()), order_list_hash,self.soup_delivered)
        )
    ...
    def to_dict(self):
        return {
            ...
            "all_orders" : [order.to_dict() for order in self.all_orders],
            "timestep" : self.timestep,
            "soup_delivered": self.soup_delivered
        }
...
class OvercookedGridworld(object):
    ...
    def __init__(self, terrain, start_player_positions, start_bonus_orders=[], rew_shaping_params=None, layout_name="unnamed_layout", start_all_orders=[], num_items_for_soup=3, order_bonus=2, start_state=None, **kwargs):
        ...
        start_all_orders: List of all available order dicts the players can make, defaults to all possible recipes if empy list provided
    ...
    def get_state_transition(self, state, joint_action, display_phi=False, motion_planner=None):
        ...
        # do nothing if goal state reached

        if state.soup_delivered:
            infos = {
            ...
            for player in range(self.num_players):
                if joint_action[player] != Action.STAY:
                    infos["shaped_reward_by_agent"][player] = 0 # make this -1 to force to stay in goal state
    ...
    def resolve_interacts(self, new_state, joint_action, events_infos):
        ...
        for player_idx, (player, action) in enumerate(zip(new_state.players, joint_action)):
            ...
            if terrain_type == 'X': # In this simplification, objects can't be dropped at the counter
                return sparse_reward, shaped_reward # this will be removed
                ...
                if self.soup_to_be_cooked_at_location(new_state, i_pos):
                    ...
                    # cook only acceptable orders
                    sorted_ingreds = sorted(soup.ingredients)
                    if any(sorted_ingreds== sorted(order['ingredients']) for order in self.start_all_orders):

                        soup.begin_cooking()
                        shaped_reward[player_idx] += self.reward_shaping_params["BEGIN_COOKING_REWARD"]
                ...
                if player.get_object().name == 'dish' and self.soup_ready_at_location(new_state, i_pos):
                    ...
                    if not soup.is_full:
                        ...
                        temp_soup = soup.deepcopy()

                        temp_soup.add_ingredient(player.get_object())

                        if self.soup_can_make_valid_recipe(temp_soup): # check if adding the ingredient isn't useless 
                            obj = player.remove_object()
                            soup.add_ingredient(obj)
                            if obj.name == Recipe.ONION and "ONION_POT_PLACEMENT" in self.reward_shaping_params: 
                                shaped_reward[player_idx] += self.reward_shaping_params["ONION_POT_PLACEMENT"]
                            elif obj.name == Recipe.TOMATO and "TOMATO_POT_PLACEMENT" in self.reward_shaping_params:
                                shaped_reward[player_idx] += self.reward_shaping_params["TOMATO_POT_PLACEMENT"]
                            else:
    ...
    def count_soup_ingredients(self, order):
        ingredients_dict = Counter(order)
        ingredient_list = []
        for ingredient in Recipe.ALL_INGREDIENTS:
            ingredient_list.append(ingredients_dict[ingredient])
        return tuple(ingredient_list)
    
    # def soup_can_make_valid_recipe(self, soup):
    #     if len(soup.ingredients) == 1: return True # no need to check if only one ingredient is added
    #     if  {'ingredients':soup.ingredients} in self.start_all_orders:
    #         return True
    #     else: # can it make valid order by considering neighbors?
    #         neighbors = Recipe(soup.ingredients).neighbors()
    #         for neighbor in neighbors:
    #             if  {'ingredients':list(neighbor.ingredients)} in self.start_all_orders:
    #                 return True
    #         return False
    ...
    def soup_can_make_valid_recipe(self, soup):
        ...
        if len(soup.ingredients) == 1: return True  # no need to check if only one ingredient is added
        sorted_ingredients = sorted(soup.ingredients)
        
        sorted_start_all_orders =  [{'ingredients': sorted(order['ingredients'], key=lambda ingredient: ingredient.lower())} for order in self.start_all_orders]

        visited = []  # keep track of visited ingredients to avoid visiting them again
        queue = deque()  # use a queue to keep track of ingredients to visit
        queue.append(sorted_ingredients)

        while queue:
            curr_ingredients = queue.popleft()
            if {'ingredients': curr_ingredients} in sorted_start_all_orders:
                return True
            if curr_ingredients in visited:
                continue
            visited.append(curr_ingredients)
            neighbors = Recipe(curr_ingredients).neighbors()
            sorted_neighbors = [sorted(neighbor.ingredients) for neighbor in neighbors]
    ...
    def count_soup(self, order):
        """Returns a tuple with the counts of each ingredient in the order of Recipe.ALL_INGREDIENTS"""
        ingredient_list = []
        ingredients_dict = Counter(order)
        for ingredient in Recipe.ALL_INGREDIENTS:
            ingredient_list.append(ingredients_dict[ingredient])
    ...
    def deliver_soup(self, state, player, soup):
        ...
        # if len(state.all_objects_list) == 0: # no object left in environment
        state.mark_soup_delivered()
...
#     def get_all_possible_states(self, agent_idx=0, debug=False):
#         # We impose the constraint that there can only be 1 total soup in the environment and then the soup can be
#         # delivered by the player.

#         players = 1
#         start_state = self.get_standard_start_state()
#         all_orders = list(map(lambda x: tuple(list(x.values())[0]), self.start_all_orders))
        
#         all_order_states = list(itertools.product([0, 1], repeat=len(all_orders)))
        
#         # get all positions 
#         pos = copy.deepcopy(self.get_valid_player_positions())
#         # print("Player Positions: ",len(pos))

#         start_state = self.get_standard_start_state()
#         if players >1:
#             other_agent_pos = start_state.players[1-agent_idx].position
#             if other_agent_pos in pos:
#                 pos.remove(other_agent_pos)

#         dir = Direction.ALL_DIRECTIONS

#         # Gets the agent position and direction
#         pos_dir = list(itertools.product(pos, dir))
#         # print("Player Pos + Dir: ",len(pos_dir))

#         player_soup = self.get_valid_player_soup_objects(all_orders)
        
#         # Add cooking state to the ready soup - The cooking state is false since this soup is already cooked
#         player_soup = list(itertools.product(player_soup,[False]))
        
#         player_object = [None] + Recipe.ALL_INGREDIENTS + ['dish'] + player_soup
#         # print("Player Objects: ",len(player_object))
#         player = list(itertools.product(pos_dir, player_object))
#         # print("Player + Objects: ",len(player))

#         pot_soup = self.get_valid_pot_soup_objects()
       
#         can_cook_soup = self.get_valid_cooking_soup_objects(pot_soup)

#         # Add cooking state to the soups
#         soups_not_ready = list(itertools.product(pot_soup,[False]))+list(itertools.product(can_cook_soup,[True]))

        

#         # We make the assumption of only two soups being cooked at a time
#         pots = list(itertools.product([None] + player_soup + soups_not_ready, repeat=self.num_pots))
        
#         # Only two objects allowed in the environment at a time

#         # ((player loc, player dir), (object), (pots with possible objects))

#         # We want a tuple of player position, direction, and object they are holding, then if there is a pot with a cooking soup, the ingredients, and if the soup is ready
#         all_states_tuples = list(itertools.product(list(itertools.product(player, pots)),[True, False]))
#         all_states = self.construct_states(all_states_tuples)
#         print("There are",len(all_states)," valid states in the current setup")
#         return all_states


    def get_all_possible_states(self, agent_idx=0, debug=False):

        # We impose the constraint that there can only be 1 total soup in the environment and then the soup can be delivered by the player.
        # Set the number of players to 1 and get the standard start state of the game.
        players = 1
        start_state = self.get_standard_start_state()

        # Get all the possible orders.
        all_orders = list(map(lambda x: tuple(list(x.values())[0]), self.start_all_orders))

        # Get all possible order states.
        all_order_states = list(itertools.product([0, 1], repeat=len(all_orders)))

        # Get all valid player positions.
        pos = copy.deepcopy(self.get_valid_player_positions())

        # If there are more than one player, remove the other player's position from the list of positions.
        if players > 1:
            other_agent_pos = start_state.players[1-agent_idx].position
            if other_agent_pos in pos:
                pos.remove(other_agent_pos)

        # Get all possible player positions and directions.
        dir = Direction.ALL_DIRECTIONS
        pos_dir = list(itertools.product(pos, dir))

        # Get all valid player soup objects.
        player_soup = self.get_valid_player_soup_objects(all_orders)
        ...
        # Get all possible combinations of player positions, directions, and objects.
        player = list(itertools.product(pos_dir, player_object))

        # Get all valid pot soup objects.
        pot_soup = self.get_valid_pot_soup_objects()

        # Get all valid cooking soup objects.
        can_cook_soup = self.get_valid_cooking_soup_objects(pot_soup)
        ...
        # We make the assumption of only two soups being cooked at a time.
        # Get all possible combinations of pots and soup objects.
        pots = list(itertools.product([None] + player_soup + soups_not_ready, repeat=self.num_pots))

        # Only two objects allowed in the environment at a time.
        # Get all possible combinations of player, pots with possible objects, and whether the soup is ready or not.
        all_states_tuples = list(itertools.product(list(itertools.product(player, pots)),[True, False]))

        # Construct states from the tuples.
        all_states = self.construct_states(all_states_tuples)

        # Print the number of valid states in the current setup.
        # print("There are",len(all_states)," valid states in the current setup")
    ...
    def construct_states(self,state_tuples): # constructs valid overcoked states from the numerical tuple state representations 
        states = state_tuples
        
        oc_states = []

        pot_location = self.get_pot_locations()[0] # pick the first pot - this setup considers a single pot
        for i in range(len(states)):
            # def construct_valid_state(data):
            state = {}

            #extract and construct a valid Overcooked State from a Tuple representation
            player_objects = None
            player_pos = states[i][0][0][0][0]
            if states[i][0][0][1] != None:
                player_object = states[i][0][0][1] # a player holds a single object
                if player_object in ["tomato","onion","dish"]:
                    player_objects =ObjectState(player_object,player_pos).to_dict()
                else: # it's soup object
                    soup_tuple = states[i][0][0][1]
                    num_t = soup_tuple[0][0][1] # tomatos
                    num_o = soup_tuple[0][0][0] # onions
                    cooking_tick = 4 if soup_tuple[1] else -1
                    player_objects=SoupState.get_soup(player_pos,  
                                                        num_tomatoes=num_t,num_onions=num_o, finished=True).to_dict()


            player = {
                "position": states[i][0][0][0][0],
                "orientation": states[i][0][0][0][1],
                "held_object":player_objects
            }

            objects = []
            for soup_obj in states[i][0][1]:
                if soup_obj != None:
                    soup_tuple = soup_obj
                    num_t = soup_tuple[0][0][1] # tomatos
                    num_o = soup_tuple[0][0][0] # onions
                    cooking_tick = 4 if soup_tuple[1] else -1
                    objects.append(SoupState.get_soup(pot_location, num_onions=num_o, 
                                                        num_tomatoes=num_t, finished=soup_tuple[0][1],cooking_tick=cooking_tick).to_dict())


            state["players"] = [player]
            state["objects"] = objects
            state["all_orders"]= self.start_all_orders
            state["bonus_orders"]=  self.start_bonus_orders
            state["timestep"] = -1 # -1 means that this value should be ignore
            state["soup_delivered"] = states[i][1] 

            overcooked_state = OvercookedState.from_dict(state)
            if state["soup_delivered"]:
                if player['held_object'] != None   : #impossible state to reach
                    continue 
            elif ~state["soup_delivered"]: # check if there are more than two soups -- impossible to reach state
                if len(overcooked_state.all_objects_by_type["soup"])>1:
                    continue

            oc_states.append(overcooked_state)
        return oc_states
    ...
    def get_valid_pot_soup_objects(self, recipes=None):
        '''All valid soup objects with a parameter for whether the soup is ready or not (no cooking state)'''

        if not recipes:
            recipes = Recipe.ALL_RECIPES

        recipes = list(map(self.count_soup, recipes))

        soup_objects = list(itertools.product(recipes, [False]))
        return soup_objects
    def get_valid_cooking_soup_objects(self, all_soups):
        '''
            - Identifies the soups that can go in cooking state
            - only soups with MAX INGREDIENTS CAN COOK
        '''
        can_cook = []
        for soup in all_soups:
            if soup[0][0] + soup[0][1] >= 3: # this can be modified to allow cooking of recipes with less than 3 ingredients
                can_cook.append(soup)
        return can_cook
        

    def get_valid_player_soup_objects(self, recipes=None):
        '''Players can only hold ready soups'''
        if not recipes:
            recipes = Recipe.ALL_RECIPES

        recipes = list(map(self.count_soup, recipes))
        
        soup_objects = list(itertools.product(recipes, [True]))
        return soup_objects
    def get_valid_player_positions_bfs(self, pos):
        visited = set()
        queue = [pos]
        ...
        while queue:
            ...
            for dir in Direction.ALL_DIRECTIONS:
                neighbor = tuple(sum(x) for x in zip(pos, dir))
                if neighbor not in visited and self.get_terrain_type_at_pos(neighbor) == ' ':
                    queue.append(neighbor)
            visited.add(pos)
        return list(visited)
