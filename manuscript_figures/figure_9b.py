...
SOURCE_PDF = PLOT_DIR / "grid6" / PLOT_MODEL_ID / "wcd_reduction" / "grid6_wcd_reduction_uniform_cost.pdf"
...
def run_generation() -> Path:
    ...
    if _has_raw_data(grid_size=6):
        print("[figure_9b] Running prepare_results_for_analysis_optimal.py (grid 6)", flush=True)
        _run(prep_cmd, description="data aggregation")
    else:
        print("[figure_9b] Raw optimal data trimmed; using precomputed summaries.")

    print("[figure_9b] Running analyze_and_plot_optimal.py (grid 6)", flush=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Figure 9b.")
    parser.add_argument(
        "--preflight-only",
        action="store_true",
    ...
    if args.preflight_only:
        ...
        if not ok:
            ...
            raise SystemExit(
                "Figure 9b preflight failed. Missing inputs:\n"
                f"{formatted}"
            )
        print("[figure_9b] Preflight checks passed.")
    else:
        main()
