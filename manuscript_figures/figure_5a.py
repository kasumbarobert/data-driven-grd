"""
Driver script for Figure 5a (Overcooked-AI WCD reduction).

This figure corresponds to the Overcooked-AI panel in Figure 5 of the paper.
It invokes the data-preparation and plotting scripts directly, then stages the
...
def _run(cmd: Sequence[str], cwd: Path, env: dict[str, str], description: str) -> None:
    try:
        ...
        print(f"[figure_5a] Skipping {description}; expected data directory missing.")
...
def main() -> None:
    ...
    target = GENERATED_DIR / "Figure_5a.pdf"
    GENERATED_DIR.mkdir(parents=True, exist_ok=True)
    shutil.copy2(source, target)
    print(f"Figure 5a copied to {target}")
