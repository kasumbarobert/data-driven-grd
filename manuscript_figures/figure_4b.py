...
SOURCE_PDF = PLOT_DIR / "grid13" / PLOT_MODEL_ID / "wcd_reduction" / f"grid13_wcd_reduction_ratio_{RATIO_ID}.pdf"
...
def run_generation() -> Path:
    ...
    if _has_raw_data(grid_size=13):
        print("[figure_4b] Running prepare_results_for_analysis_optimal.py (grid 13)", flush=True)
        _run(prep_cmd, description="data aggregation")
    else:
        print("[figure_4b] Raw optimal data trimmed; using precomputed summaries.")

    print("[figure_4b] Running analyze_and_plot_optimal.py (grid 13)", flush=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Figure 4b.")
    parser.add_argument(
        "--preflight-only",
        action="store_true",
    ...
    if args.preflight_only:
        ...
        if not ok:
            ...
            raise SystemExit(
                "Figure 4b preflight failed. Missing inputs:\n"
                f"{formatted}"
            )
        print("[figure_4b] Preflight checks passed.")
    else:
        main()
