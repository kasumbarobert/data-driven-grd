"""Driver for Figure 9a (6×6 grid, individual budgets)."""
...
SOURCE_PDF = PLOT_DIR / "grid6" / PLOT_MODEL_ID / "wcd_reduction" / f"grid6_wcd_reduction_ratio_{RATIO_ID}.pdf"
...
def _run(cmd: Sequence[str], description: str) -> None:
    try:
        ...
        print(f"[figure_9a] Skipping {description}; expected data directory missing.")


def run_generation() -> Path:
    """Regenerate the 6×6 individual-budget panel and label it."""
    run_preflight_checks(
        figure_tag="figure_9a",
        root=ROOT,
    ...
    prep_cmd = [PYTHON, "prepare_results_for_analysis_optimal.py", "--grid_size", "6"]
    plot_cmd = [
        PYTHON,
    ...
    if _has_raw_data(grid_size=6):
        print("[figure_9a] Running prepare_results_for_analysis_optimal.py (grid 6)", flush=True)
        _run(prep_cmd, description="data aggregation")
    else:
        print("[figure_9a] Raw optimal data trimmed; using precomputed summaries.")

    print("[figure_9a] Running analyze_and_plot_optimal.py (grid 6)", flush=True)
    _run(plot_cmd, description="plot generation")

    labelled_output = PAPER_FIGURES_DIR / "figure_9a.pdf"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Figure 9a.")
    parser.add_argument(
        "--preflight-only",
        action="store_true",
    ...
    if args.preflight_only:
        ...
        if not ok:
            ...
            raise SystemExit(
                "Figure 9a preflight failed. Missing inputs:\n"
                f"{formatted}"
            )
        print("[figure_9a] Preflight checks passed.")
    else:
        main()
