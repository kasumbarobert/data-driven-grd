...
def compute_true_wcd(x, model= None):
    grid_size, goal_positions, blocked_positions, start_pos,space_pos = decode_grid_design(x)
    return compute_human_wcd(grid_size, goal_positions, blocked_positions, start_pos,model = model)
...
def check_design_is_valid(x,is_grid = True):
    if is_grid:
...
def filter_action(layout):
    layout = layout.reshape(4,6,6).numpy()
...
def compute_human_wcd(grid_size, goal_positions, blocked_positions, start_pos, model= None):
    designs = []
    for i,goal_pos in enumerate(goal_positions):
        temp_blocked_positions = blocked_positions +[goal_positions[1-i]] # the other goal's  position is blocked
    ...
    for i, layout in enumerate(designs):
        move, flag = compute_human_path(layout, model)
        moves.append(move)
        flags.append(flag)
    if not flags[0] and not flags[1]:
        wcd = find_wcd(moves[0], moves[1])
    else:
        wcd = None
    return wcd

def compute_human_path(layout, model):

    # layout = designs[0][0,:,:,:]
    max_step = 20 # fail to reach goal if reach max_step
    cur_layout = layout.reshape([1,4,6,6]).detach().clone()
    cur_pos = tuple(torch.argwhere(cur_layout[0,2,:,:]).detach().numpy()[0])
    move = []
    for _ in range(max_step):
        action_flag, valid_action = filter_action(cur_layout)
        if action_flag != 4: # 1 action away
            move.append(action_flag) 
            break
        human_pred = model.forward(cur_layout).detach().numpy().reshape(-1)
        human_pred = human_pred * valid_action
